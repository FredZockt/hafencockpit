...
/**
 * Konfiguration des Schiffsverkehr-Widgets (AIS).
 *
 * `proxyBaseUrl` statt einer Anbieter-URL: AIS ist die einzige Quelle, die nicht
 * direkt aus dem Browser abrufbar ist (API-Key + kein CORS). Es gibt hier daher
 * bewusst KEIN Feld für einen API-Key - der gehört ins Backend, nie ins Bundle.
 */
export interface AisConfig {
  /** Basis-URL unseres eigenen Proxys. Wird vom Mock-Repository nicht benutzt. */
  readonly proxyBaseUrl: string;
  /** Suchradius um den Hafenstandort. */
  readonly radiusKm: number;
  /** Obergrenze der angezeigten Schiffe - eine Kachel ist kein Radarbild. */
...
export const DEFAULT_HARBOR_COCKPIT_CONFIG: HarborCockpitConfig = {
  ...
  ais: {
    proxyBaseUrl: 'http://localhost:8080/api/ais',
    radiusKm: 25,
    maxVessels: 25,
  },
