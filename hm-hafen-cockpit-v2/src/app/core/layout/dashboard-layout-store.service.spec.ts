...
describe('DashboardLayoutStore', () => {
  ...
  it('lädt das Layout beim Start aus dem Repository', () => {
    // Gegen das Default abgeglichen statt gegen eine feste Liste: Ein neues Widget
    // im Auslieferungszustand soll diesen Test nicht brechen.
    expect(store.widgets().map((widget) => widget.type)).toEqual(
      DEFAULT_DASHBOARD_LAYOUT.widgets.map((widget) => widget.type),
    );
  });
  ...
  it('verschiebt eine Kachel und nummeriert die Positionen neu', () => {
    const before = store.widgets().map((widget) => widget.id);

    store.moveWidget(0, 2);

    const widgets = store.widgets();

    // Erste Kachel ist an Position 2 gewandert, der Rest rückt auf.
    const expected = [...before];
    const [moved] = expected.splice(0, 1);
    expected.splice(2, 0, moved as string);
    expect(widgets.map((widget) => widget.id)).toEqual(expected);

    // Positionen wieder lückenlos - der persistierte Stand trägt die Reihenfolge selbst.
    expect(widgets.map((widget) => widget.position)).toEqual(widgets.map((_, index) => index));
  });
  ...
  it('entfernt eine Kachel und persistiert', () => {
    const erwartet = DEFAULT_DASHBOARD_LAYOUT.widgets
      .map((widget) => widget.id)
      .filter((id) => id !== 'weather-main');

    store.removeWidget('weather-main');

    expect(store.widgets().map((widget) => widget.id)).toEqual(erwartet);
    // Positionen lückenlos neu vergeben.
    expect(store.widgets().map((widget) => widget.position)).toEqual(erwartet.map((_, i) => i));
    expect(repository.saveCount).toBe(1);
  });
  ...
  it('ignoriert das Entfernen einer unbekannten ID', () => {
    ...
    expect(store.widgets()).toHaveLength(DEFAULT_DASHBOARD_LAYOUT.widgets.length);
    expect(repository.saveCount).toBe(0);
  });
  ...
  it('bietet nur Widget-Typen an, die noch nicht auf dem Board liegen', () => {
    // Das Default-Layout enthält alle Typen -> nichts mehr anzubieten.
