...
export const WIDGET_REGISTRY: Record<WidgetType, WidgetDefinition> = {
   ...
   * SCHIFFSVERKEHR (AIS)
   *
   * Die einzige Quelle, die NICHT direkt aus dem Browser abrufbar ist: Jeder
   * AIS-Anbieter verlangt einen Key, und aisstream.io schließt Browser-
   * Verbindungen ausdrücklich aus (kein CORS). Deshalb läuft dieses Widget
   * derzeit gegen ein Mock-Repository - der Umstieg auf einen Proxy ist eine
   * Zeile in app.config.ts. Siehe README.
   */
  'ship-traffic': {
    type: 'ship-traffic',
    title: 'Schiffsverkehr Elbe',
    description: 'Ein- und auslaufende Schiffe im Umkreis des Hafens (AIS).',
    dataSource: 'AIS (Demo-Daten)',
    loadComponent: () =>
      import('../../features/ship-traffic/ui/ship-traffic-widget.component').then(
        (m) => m.ShipTrafficWidgetComponent,
      ),
  },
};
